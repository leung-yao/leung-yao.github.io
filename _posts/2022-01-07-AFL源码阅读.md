...
![image-20211220133226189](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211220133226189.png)

![image-20211220133527406](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211220133527406.png)
...
- 进入一个while循环，开始遍历之前生成的input queue
  ...
  - ![image-20211222162045692](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211222162045692.png)

  - 用strrchr找到`id:00000.......`也就是文件名，返回给fn

  - 输出一串字符串![image-20211222162608006](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211222162608006.png)
  ...
  - 返回值为 `crash_mode` 或者 `FAULT_NOBIT`时
    ...
    - 打印`SAYF("len = %u, map size = %u, exec speed = %llu us\n", q->len, q->bitmap_size, q->exec_us);`![image-20211222171312118](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211222171312118.png)
...
- 接着进入一个大for循环中：`for (stage_cur = 0; stage_cur < stage_max; stage_cur++)`
  ...
  - 调用**write_to_testcase(use_mem, q->len)**将读取的内容写入我们的out_file中
    ...
    - 如果out_file存在：
      - ![image-20211223164837756](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211223164837756.png)
...
- 然后就会展示一些信息
- ![image-20211228134733498](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211228134733498.png)
...
- 统计子进程的资源用量并写入
- <img src="https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211228141429672.png" alt="image-20211228141429672" style="zoom:200%;" />
...
- 如果fault == crash_mode
  ...
  - 利用hash32从新计算trace_bits的哈希值，将其设置为`queue_top->exec_cksum`
    - ![image-20211228194053173](https://gitee.com/gdmzyzl/picgo/raw/master/picbed/image-20211228194053173.png)
